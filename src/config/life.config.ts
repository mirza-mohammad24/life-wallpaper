...
export const loadConfig = (): UserConfig => {
  try {
    ...
    const safeToReturnConfig : UserConfig = {
      ...
      dob: typeof safeObject.dob === 'string' 
      ? safeObject.dob 
      : defaultConfig.dob,

      expectancy: typeof safeObject.expectancy === 'number' 
      ? safeObject.expectancy 
      : defaultConfig.expectancy,

      message: typeof safeObject.message === 'string' 
      ? safeObject.message 
      ...
      shape: isShape(safeObject.shape)
      ? safeObject.shape 
      : defaultConfig.shape,
    ...
    console.error("Failed to retrieve data from localStorage returning the defaultConfig.", error);
...
export const saveConfig = (config: UserConfig): void => {
  try {
    ...
    console.error("Saving failed. Continuing to run the app on previous Configuration", error);
  }
};
